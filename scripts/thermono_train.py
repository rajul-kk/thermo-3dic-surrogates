...
def run_fold(D, tr, te, args):
    ...
    model = ThermoNO(grid, ch=args.ch, n_blocks=args.blocks, modes=(args.modes, args.modes),
                     basis=args.basis, geo_arch=args.geo_arch, geo_attn=args.geo_attn)
...
def main():
    ...
    ap.add_argument('--basis', choices=['dct', 'walsh'], default='dct',
                    help='Spectral basis for the linear path (ablation of SPFNO/Hartley-NO claim).')
    ap.add_argument('--geo-arch', choices=['mlp', 'cno'], default='mlp',
                    help='Geometry-encoder architecture: plain conv (mlp) or CNOFNOHybrid-style '
                         'multiscale encode/decode (cno).')
    ap.add_argument('--geo-attn', action='store_true',
                    help='Add axial self-attention at the geo-arch=cno bottleneck.')
    args = ap.parse_args()
    ...
    tag = args.basis + ('-cno' + ('-attn' if args.geo_attn else '') if args.geo_arch == 'cno' else '')
    out = Path(f'results/thermono_{args.geometry}_{tag}_seed{args.seed}.json')
