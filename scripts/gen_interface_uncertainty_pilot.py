...
from src.core.geometry_builders import build_geometry5
...
# (layer_name, [values]) -- nominal value listed first for each.
SWEEPS = {
...
def main():
    ...
    for layer_name, values in SWEEPS.items():
        for value in values:
            ...
            sc_dict['layer_k_overrides'] = {layer_name: value}
            suffix = f"_{args.tag}" if args.tag else ""
            scenario_name = f"{GEOM_NAME}_iface_{layer_name}_{value:g}{suffix}"
