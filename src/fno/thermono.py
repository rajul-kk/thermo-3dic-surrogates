...
#  - optional Walsh-Hadamard basis, reusing this repo's own WHNO block (whno.py::WalshConv3d)
#    unmodified as an ablation of the DCT choice above -- real and linear (so the linear path
#    stays linear) but not adapted to adiabatic walls, the SPFNO/Hartley claim this ablation checks
#  - dense learned z-mixing instead of spectral z: the discrete transfer-matrix coupling of layers
#  - optional CNO-style multiscale encode/decode (+ axial attention), borrowed from this repo's
#    CNOFNOHybrid, applied ONLY inside the geometry encoder -- it is power-free and already meant
#    to be nonlinear, so a strided-conv/attention encoder cannot break the dT(a*q)=a*dT(q) guarantee
...
class DCTSpectralConv(nn.Module):
    """Linear map: DCT in x and y (the Neumann eigenbasis for this benchmark's adiabatic walls,
    matching the layered backbone, report §9.25), per-mode channel mixing, dense mixing across z
    (the discrete transfer-matrix coupling), inverse DCT."""
...
def make_spectral_block(basis: str, ch: int, grid, modes):
    """`basis='dct'` uses DCTSpectralConv (matches the layered backbone's boundary condition).
    `basis='walsh'` reuses this repo's own WHNO block (`src/fno/whno.py::WalshConv3d`) unmodified
    -- real, linear, but not adapted to adiabatic walls, so this is a direct ablation of the
    SPFNO/Hartley-NO claim that basis choice should follow the operator's boundary symmetry."""
    nx, ny, nz = grid
    mx, my = modes
    if basis == 'dct':
        return DCTSpectralConv(ch, nx, ny, nz, mx, my)
    if basis == 'walsh':
        from .whno import WalshConv3d
        return WalshConv3d(ch, ch, grid, (mx, my, nz))     # keep full z-rank; only x,y are truncated
    raise ValueError(f'unknown basis {basis!r}; expected "dct" or "walsh"')
...
class ThermoNO(nn.Module):
    ...
    def __init__(self, grid, ch: int = 16, n_blocks: int = 3, modes=(16, 16),
                 lin_in: int = 2, geo_in: int = 4, basis: str = 'dct',
                 geo_arch: str = 'mlp', geo_attn: bool = False):
        super().__init__()
        nx, ny, nz = grid
        # No bias anywhere on the linear path: a bias would break dT(a*q) = a*dT(q). `basis` and
        # `geo_arch`/`geo_attn` are ablation switches (report §9.26): basis='walsh' swaps the
        # DCT spectral layers for a Walsh-Hadamard basis of the same shape; geo_arch='cno' swaps
        # the plain conv geometry encoder for CNOFNOHybrid's multiscale encoder/decoder, and
        # geo_attn adds axial self-attention at its bottleneck. Neither touches the linear path.
        self.lift = nn.Conv3d(lin_in, ch, 1, bias=False)
        self.spec = nn.ModuleList([make_spectral_block(basis, ch, grid, modes) for _ in range(n_blocks)])
        self.local = nn.ModuleList([nn.Conv3d(ch, ch, 1, bias=False) for _ in range(n_blocks)])
        n_gates = 2 * n_blocks + 1
        if geo_arch == 'mlp':
            self.geo = GeometryEncoder(geo_in, ch, n_gates)
        elif geo_arch == 'cno':
            self.geo = CNOGeometryEncoder(geo_in, ch, n_gates, grid, use_attention=geo_attn)
        else:
