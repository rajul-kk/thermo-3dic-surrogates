...
>
> **`kaggle_thermono_multiseed.ipynb`** (report §9.26): ThermoNO vs the classical backbone,
> seeds 0–2, on whichever of `geometry{4,5,6}_shelf_v5.zip` are attached. Internet On, GPU T4.
> It clones the repo and runs `scripts/thermono_train.py` unchanged. Download
> `/kaggle/working/thermono_results/` at the end.
...
| `kaggle_thermono_multiseed.ipynb` | ThermoNO vs the training-free layered backbone (report §9.26), 5-fold CV × seeds 0–2 on geometry4/5/6; paired per test scenario. | any of `data/kaggle_v5/geometry{4,5,6}_shelf_v5.zip` |
